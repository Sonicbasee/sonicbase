      ...
      <SectionCard title="Artist roster" eyebrow="Management">
        ...
        <TableCard
          ...
          rows={artists.map((artist) => ({
            ...
            actions: <div className="flex gap-2"><Button asChild variant="secondary" size="sm"><Link to="/admin/artists/$artist/edit" params={{ artist: artist.id }}>Edit</Link></Button><Button variant="destructive" size="sm" onClick={() => { if (confirm("Delete this artist?")) deleteMutation.mutate(artist.id); }}>Delete</Button></div>,
          }))}
        />
        )}
      </SectionCard>
